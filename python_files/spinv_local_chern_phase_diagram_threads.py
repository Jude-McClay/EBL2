...
if __name__ == "__main__":
    ...
    # pool.close()
    # pool.join()
    t1 = ReturnableThread(target = lcm_calc, args = (0.2,))
    t2 = ReturnableThread(target = lcm_calc, args = (2,))
    t3 = ReturnableThread(target = lcm_calc, args = (1,))
    t1.start()
    t2.start()
    t3.start()
    a = t1.join()
    ...
    thread_list = []
    for m in masses:
        t = ReturnableThread(target = lcm_calc, args=(m,))
        t.start()
